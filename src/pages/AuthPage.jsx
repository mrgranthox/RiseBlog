...
const AuthPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const toastId = toast.loading(
      state === "Sign Up" ? "Registering..." : "Logging in..."
    );

    try {
      const endpoint =
        state === "Sign Up" ? "/auth/user/register" : "/auth/user/login";
      const payload =
      ...
      if (!navigator.onLine) {
        ...
        toast.error(
          "No response from server. Please check internet connection and try again.",
          { id: toastId }
        );
      } else {
