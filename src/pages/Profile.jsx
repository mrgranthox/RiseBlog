...
const Profile = () => {
  const { userData, getPostByUser, isLoggedIn, postByUser } =
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-12 gap-12 relative z-10">
          ...
          >
            <div className="relative w-32 h-32 mx-auto">
              <img
                src={userData?.profilePicture || "/RISEBLOG.svg"}
                alt="Profile"
                className="relative z-10 rounded-full border-4 border-white shadow-lg object-cover w-full h-full"
              />
                    ...
                    {getPostByUser.map((post, i) => (
                      ...
                      >
                        <div className="relative">
                          <img
                            src={post?.coverImage || "/RISEBLOG.svg"}
                            alt={post?.title}
                          ...
                          <div className="absolute top-3 right-3 z-20">
                            ...
                            {showActionsFor === i && (
                              <div className="absolute right-0 mt-2 w-40 rounded-lg bg-black/70 backdrop-blur-xl shadow-2xl border border-white/20 overflow-hidden z-30">
                                <div
                                  onClick={() =>
                                    navigate(`/articles/post/${post.slug}`)
                                  }
                                  className="flex items-center gap-2 px-4 py-3 text-white hover:bg-white/20 cursor-pointer transition-colors"
                                >
                                  <Eye size={16} />
                                  <span>View</span>
                                </div>
                                <div
                                  onClick={() =>
                                    navigate(`/profile/edit-post/${post.id}`)
                                  }
                                  className="flex items-center gap-2 px-4 py-3 text-white hover:bg-white/20 cursor-pointer transition-colors"
                                >
                                  <Edit size={16} />
                                ...
                                <div
                                  ...
                                  className="flex items-center gap-2 px-4 py-3 text-red-300 hover:bg-red-500/40 cursor-pointer transition-colors"
                                >
                                  <Trash2 size={16} />
                                  <span>Delete</span>
                                </div>
